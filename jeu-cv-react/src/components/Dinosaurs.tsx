...
import {dinoSpeed, intervalSpeedDino, windowSize} from "../constants/contants";
import {useInterval} from "../helpers/hooks";
import {useGameData} from "../store/GameProvider";
import {MOVE_DINO} from "../constants";
import Explosion from "./Explosion";
import DinosaurSprite from "./SpriteElement"
import {conditionToConflict, takeSoundChoice} from "../helpers/ennemies_helpers";
...
const Dinosaurs = ({id, x = windowSize, y, width, widthDead = 0, height, avatar, spriteX = [], spriteY = [], spriteXDead = [], className, idSound = '', speed, alive}: IPropsDino) => {
    ...
    useInterval(() => {
        const positionHero = positionInitial.current === -width ? player.x : player.x + player.width-30
